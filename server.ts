...
const defaultSessions: ChatSession[] = [
  ...
  },
  {
    id: "session-emp-1",
    role: "employee",
    title: "Обучение и командировки",
    date: "05.06.2026",
    messages: [
      {
        id: "m5",
        role: "user",
        content: "Привет! Какие курсы профессионального развития нам доступны от СберУниверситета?",
        timestamp: "05.06.2026, 10:15:32"
      },
      {
        id: "m6",
        role: "assistant",
        content: "Здравствуйте! Для сотрудников компании **ООО 'ТехноПром'** доступны бесплатные программы обучения в СберУниверситете по темам цифрового развития и бизнес-аналитики. Вы можете ознакомиться с ними на боковой панели справа.",
        timestamp: "05.06.2026, 10:16:15"
      }
    ]
  }
];
...
app.post("/api/sessions/new", (req, res) => {
  ...
  const newSession: ChatSession = {
    id: "session-" + Date.now(),
    role: role || "director",
    title: title || (role === "director" ? "Новая консультация владельца" : role === "accountant" ? "Новый запрос бухгалтера" : "Новый запрос сотрудника"),
...
function generateMockResponse(role: string, prompt: string, isOperatorRequest: boolean, currentUrl: string, detectedTerms: string[]): { content: string; callOperator: boolean; operatorSummary: any; detectedTerms: string[] } {
  ...
  if (role === "employee" && (prompt.toLowerCase().includes("остаток") || prompt.toLowerCase().includes("баланс") || prompt.toLowerCase().includes("счет") || prompt.toLowerCase().includes("деньги") || prompt.toLowerCase().includes("лимит") || prompt.toLowerCase().includes("выписк"))) {
    reply = "🔒 **Доступ заблокирован ограничениями роли 'Сотрудник'**. По соображениям корпоративной безопасности, просмотр балансов юридического лица, выписок и активированных кредитных лимитов доступен только Владельцу бизнеса или Главному бухгалтеру компании. Обратитесь к руководителю за получением финансовых сведений.";
  } else if (isOperatorRequest) {
    ...
    if (role === "director") {
      ...
      reply = "Приветствую! Как сотрудник компании, я могу подсказать вам полезные банковские услуги для персонала: подключение СберБизнес Командировок, покупку гостиниц и билетов со скидкой 10% или прохождение курсов повышения квалификации в СберУниверситете. Чем могу помочь?";
    }
  }
  ...
  try {
    ...
    const offersContextText = role === "director"
      ...
      : role === "accountant"
        ...
        : "- Курсы СберУниверситета: бесплатные программы повышения квалификации сотрудников.\n- СберБизнес Командировки: бронирование билетов и гостиниц со скидками до 10%.";
...
- Роль в компании: ${role === "director" ? "Владелец бизнеса / ИП (CEO ООО 'ТехноПром')" : role === "accountant" ? "Главный Бухгалтер ИП Смирнова" : "Сотрудник ООО 'ТехноПром'"}
- Юридическое лицо: ${companyContext || (role === "director" ? "ООО 'ТехноПром'" : role === "accountant" ? "ИП Смирнова" : "ООО 'ТехноПром'")}
...
3. Сотрудник (role: "employee"): КРАЙНЕ ОГРАНИЧЕННЫЙ ДОСТУП. Просмотр балансов, лимитов, выписок или кредитных продуктов СТРОГО ЗАПРЕЩЕН. Если сотрудник спрашивает баланс, остаток или финансовые отчеты, вежливо отвечай: "Доступ ограничен правами вашей роли 'Сотрудник'. Обратитесь к руководителю за финансовыми выписками." Помогай ему с командировками, курсами СберУниверситета и общими правилами банка в Беларуси.
...
3. Персонализация: Учитывай роль. Директору предлагай кредиты, вклады, лизинг, ликвидность в BYN. Бухгалтеру — отчеты, налоги, автоматизация счетов, НСБУ, платежные поручения, регистрацию договоров в Нацбанке РБ. Сотруднику — командировки, курсы, общие справки.
